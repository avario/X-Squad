...
class DialView: UIStackView {
	...
	static private let forwardBearings: [Ship.Maneuver.Bearing] = [
		.tallonRollLeft,
		.segnorsLoopLeft,
		.turnLeft,
		.bankLeft,
	...
	static private let cellSize: CGFloat = 24
	...
	var ship: Ship? {
		didSet {
			...
			for speed in -2...5 {
				...
				speedLabel.heightAnchor.constraint(equalToConstant: DialView.cellSize).isActive = true
				speedLabel.widthAnchor.constraint(equalToConstant: DialView.cellSize).isActive = true
				
				row.addArrangedSubview(speedLabel)
				
				for bearing in DialView.forwardBearings {
					...
					if let maneuver = cellManeuver {
						...
						maneuverLabel.heightAnchor.constraint(equalToConstant: DialView.cellSize).isActive = true
						maneuverLabel.widthAnchor.constraint(equalToConstant: DialView.cellSize).isActive = true
						...
						emptyView.heightAnchor.constraint(equalToConstant: DialView.cellSize).isActive = true
						emptyView.widthAnchor.constraint(equalToConstant: DialView.cellSize).isActive = true
